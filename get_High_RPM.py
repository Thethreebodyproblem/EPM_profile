...
config = {
        ...
        'q2' : 1,
        'elimination_step' : 50,
        'elimination_upper' : 2000
        }


def get_valid_key(bucket_name):
    """
...
#def get_valid_df(look_up):
#    valid_data = set()
#    for key in look_up.keys():
#
#        try:
#            df = pd.read_csv(path+'/'+'{}_raw_port_data_for_all_trips.csv'.format(int(key)),index_col = 0)
#        except:
#            continue
...
def get_summary_df(bucket_name, path, client):
    ...
    for item in np.array(summary[['platformId','HighRPM']]):
        if item[0] not in look_up:
            ...
            print(item[0], 'multiple ground truth')
    return look_up
...
def grid_search(df,config, key, look_up):
    ...
    for i in range(0,int(max(x)),elimination_step):
        ...
        if clf.coef_[0] < 0 and cur_metric_res<metric_res:
            ...
            print('[training....] cur_metrics_res: {} best_metric_res: {} slope: {} decode: {} ' \
...
def train(valid_data,path_to_save, client, look_up, bucket_name,folder_name = 'round_2'):
...
#        print(res[-1])
    ...
    for item in res:
        if 'model' not in item: 
            print('[error] {} don\'t have model'.format(item['platform_id']))
            continue
...
def test(valid_data,path_to_load, bucket_name , client, folder_name = 'round_2', mode = 'test'):
    if not path_to_load:
        print('please provide the pre-trained parameter')
    else:
    ...
    for item in valid_data:
        ...
        print('[test...] platfor_id is {}; high_RPM is {}'.format(platform_id,score))
    df = pd.DataFrame({'platform_id':platfor_id,'high_RPM':high_RPM})
    df.to_csv(path_to_load + 'result.csv')
    
def main(args):
    valid_data, client = get_valid_key(args.bucket_name)
    look_up = get_summary_df(args.bucket_name, args.summary_name, client)
    if args.mode == 'train':
        
        
        slope, intercept = train(valid_data, args.path_to_save,client,look_up,args.bucket_name, args.folder_name)
        
        test(valid_data, args.path_to_save, args.bucket_name,client, args.folder_name, mode = 'train')
    else:
        test(valid_data, args.path_to_save, args.bucket_name, client, args.folder_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_to_save', type=str, default='./', help='path to save the result')
    parser.add_argument('--folder_name', type=str, default='round_2', help='test data folder name')
    parser.add_argument('--bucket_name', type=str, default='vnomics-external-data-analytics', help='name of the S3 bucket')
    parser.add_argument('--summary_name', type=str, default='university_of_rochester_projects/data/raw/rpm_profile/trip_data_for_rpm_profile_detection.csv', help='path for summary table')
